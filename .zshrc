...
plugins=(
    history    # 有時間戳格式的.zsh_history
    safe-paste # 避免貼上後直接執行
    colored-man-pages # 有顏色的man page
#     command-not-found # 顯示command not found的command如何獲得，會造成command not found 時，回傳的速度比較慢
    sudo # 自動加入sudo 前綴
)
...
source $DOTFILES/aliasConfig.zsh
