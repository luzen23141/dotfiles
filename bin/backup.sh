...
sync_files() {
    ...
    if [ -d "$src" ]; then
        # === 針對資料夾 ===
        if [ ! -d "$dest" ]; then
            mkdir -p "$dest" || { track_error "無法建立目錄: $dest"; return 1; }
        fi
        # 來源加斜線 = 複製內容
        if ! rsync -a "$src/" "$dest/"; then
            track_error "rsync 失敗: $src -> $dest"
            return 1
        fi
    else
        # === 針對檔案 ===
        ...
        if [ ! -d "$dest_dir" ]; then
            mkdir -p "$dest_dir" || { track_error "無法建立目錄: $dest_dir"; return 1; }
        fi
        if ! rsync -a "$src" "$dest"; then
            track_error "rsync 失敗: $src -> $dest"
            return 1
...
handle_symlink() {
    ...
    if [ "$mode" == "backup" ]; then
        ...
        if ! ln -s "$target_path" "$link_path"; then
            track_error "建立 symlink 失敗: $link_path -> $target_path"
            return 1
        fi
    fi
}
    ...
    if [ "$mode" == "backup" ]; then
        ...
        for ((j=0; j<plist_count; j++)); do
            ...
            if [ -e "$plist_path" ]; then
                ...
                echo "      複製: $plist_path"
                if ! rsync -a "$plist_path" "$backup_dir/$plist_name"; then
                    track_error "rsync 失敗: $plist_path"
                fi
            else
        ...
        for ((j=0; j<plist_count; j++)); do
            ...
            if [ -f "$backup_file" ]; then
                ...
                rsync -a "$backup_file" "$plist_path" || track_error "rsync 失敗: $backup_file -> $plist_path"
            else
                echo "      [跳過] 備份不存在: $backup_file"
                track_error "App 備份不存在: $backup_file"
            fi
        done
...
handle_export_plist_config() {
    ...
    if [ "$mode" == "backup" ]; then
        ...
        # 檢查備份檔案是否存在
        if [ ! -f "$backup_file" ]; then
            track_error "ExportPlist 備份不存在: $backup_file"
            return 1
        fi
...
backup_git_repo() {
    ...
    if [ -d "$dest_path/.git" ]; then
        ...
        if ! git clone --quiet "$url" "$dest_path"; then
            track_error "git clone 失敗: $url"
            return 1
        fi
    elif [ -d "$dest_path" ]; then
        # 資料夾存在，不是 Git 且不是空的 -> 危險
        track_error "目標目錄存在但不是 Git 倉庫且不為空，略過 Clone: $dest_path"
        return 1
    else
        # 資料夾不存在 -> 正常 Clone
...
main() {
        ...
        if [ "$mode" == "backup" ]; then
            if [ "$type" == "git" ]; then
                ...
                if [ -z "$app_name" ] || [ "$app_name" == "null" ]; then
                    track_error "App 類型必須提供 app_name"
                elif [ -z "$plist_paths" ] || [ "$plist_paths" == "null" ]; then
                    track_error "App 類型必須提供 plist_paths"
                else
                ...
                # ExportPlist 類型設定檔
                if [ -z "$app_name" ] || [ "$app_name" == "null" ]; then
                    track_error "ExportPlist 類型必須提供 app_name"
                elif [ -z "$domain" ] || [ "$domain" == "null" ]; then
                    track_error "ExportPlist 類型必須提供 domain"
                else
            ...
            if [ "$type" == "git" ]; then
                ...
                if [ -z "$target" ] || [ "$target" == "null" ]; then
                    track_error "Symlink 類型必須提供 target"
                else
                    local real_target="${target/#\~/$HOME}"
                    real_target="${real_target//\$HOME/$HOME}"
    ...
    if [ -n "$target_name" ] && [ "$processed_count" -eq 0 ]; then
        echo "錯誤: 找不到 name=$target_name 的項目" >&2
        exit 1
    fi

    if [ "$ERROR_COUNT" -gt 0 ]; then
        echo "作業結束，但有 $ERROR_COUNT 個錯誤"
        exit 1
    fi
    echo "作業完成！"
}
