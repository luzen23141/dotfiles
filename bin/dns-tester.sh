...
load_config() {
    # 僅允許已知鍵的 KEY=VALUE，避免 source 執行任意程式碼
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$line" =~ ^([A-Za-z_][A-Za-z0-9_]*)=(.*)$ ]]; then
            ...
            case "$key" in
                TIMEOUT_MS|LOOP_COUNT|BUST_CACHE)
    ...
    [[ "$TIMEOUT_MS" =~ ^[0-9]+$ ]] && [[ "$TIMEOUT_MS" -gt 0 ]] || err "設定檔 TIMEOUT_MS 必須是大於 0 的整數"
    [[ "$LOOP_COUNT" =~ ^[0-9]+$ ]] && [[ "$LOOP_COUNT" -gt 0 ]] || err "設定檔 LOOP_COUNT 必須是大於 0 的整數"
    [[ "$BUST_CACHE" =~ ^[01]$ ]] || err "設定檔 BUST_CACHE 必須是 0 或 1"
}
...
run_dns_worker() {
    ...
    write_failure_result() {
        [[ "$finished" -eq 1 || -f "$result_file" ]] && return 0
        {
            printf 'dns_ip=%s\n' "${dns_ip:-unknown}"
            printf 'provider=%s\n' "${provider:-worker-$index}"
            printf 'note=%s\n' "${note:-異常結束}"
            printf 'ping_ms=999\n'
            printf 'ping_str=超時\n'
            printf 'success=0\n'
            printf 'fail=0\n'
            printf 'total_time=0\n'
            printf 'avg_ms=999999\n'
            printf 'avg_str=超時\n'
            printf 'sort_value=999999\n'
            printf 'color_tag=R\n'
            printf 'success_rate=0%%\n'
            printf 'is_current=0\n'
        } > "$result_tmp"
        mv -f "$result_tmp" "$result_file" 2>/dev/null || true
    }
...
show_progress() {
    ...
    while [[ "$completed" -lt "$TOTAL_DNS" ]]; do
        ...
        alive=$(count_alive_workers)
        if [[ "$alive" -eq 0 ]]; then
            # 給 trap 寫入失敗結果一點時間
            sleep 0.3
            completed=$(count_completed_workers)
            if [[ "$completed" -lt "$TOTAL_DNS" ]]; then
                printf '\n' >&2
                err "DNS worker 已全部結束，但結果不完整（${completed}/${TOTAL_DNS}）"
            fi
            break
        fi
        sleep 0.2
...
print_report() {
    ...
    while IFS='|' read -r sort_value color_tag provider_display dns_ip note ping_str avg_str success_rate; do
        ...
        if [[ "$rank" -eq 1 ]]; then
            TOP_1_IP="$dns_ip"
            TOP_1_SUCCESS_RATE="$success_rate"
        elif [[ "$rank" -eq 2 ]]; then
            TOP_2_IP="$dns_ip"
        fi
...
apply_dns() {
    ...
    # 全數解析失敗時拒絕套用，避免把系統 DNS 設成無效伺服器
    if [[ "${TOP_1_SUCCESS_RATE:-0%}" == "0%" ]]; then
        printf '\n\033[31m所有 DNS 解析皆失敗（冠軍成功率 0%%），拒絕套用。\033[0m\n'
        return
    fi
